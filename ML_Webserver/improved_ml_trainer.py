...
class ImprovedMLTrainer:
    ...
    def _add_engineered_features(self, df):
        """Add engineered features for better ML performance using shared utility"""
        print("🔧 Adding engineered features...")

        # Use shared utility for consistent feature engineering
        df = FeatureEngineeringUtils.add_engineered_features_to_dataframe(df)

        # Additional trend strength features (if adx is available)
        if 'adx' in df.columns:
            df['trend_strength'] = pd.cut(df['adx'], bins=[0, 25, 50, 100], labels=['weak', 'moderate', 'strong'])

        print(f"   Added engineered features using shared utility")

        return df

    def _classify_session(self, hours):
        """Classify hours into trading sessions using shared utility"""
        return FeatureEngineeringUtils._classify_session(hours)
    ...
    def retrain_models(self, symbol: str, timeframe: str, training_data: list, models_dir: str = None) -> bool:
        ...
        try:
            ...
            # Define feature names (28 universal features including engineered features)
            feature_names = [
