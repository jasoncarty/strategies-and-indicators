...
void SaveTradeResultsForML(const TradeData &trades[], int tradeCount, string fileName, string testRunID = "", string symbol = NULL, ENUM_TIMEFRAMES tf = PERIOD_CURRENT) {
    ...
    for(int i = 0; i < tradeCount; i++) {
        ...
        totalProfit += trades[i].net_profit;
    }
...
//+------------------------------------------------------------------+
void SaveComprehensiveTradeResults(const TradeData &trades[], int tradeCount, double totalProfit, int totalTrades, int winningTrades, double winRate, string fileName, string testRunID = "", string symbol = NULL, ENUM_TIMEFRAMES tf = PERIOD_CURRENT) {
    if(symbol == NULL || symbol == "") symbol = _Symbol;
    if(tf == PERIOD_CURRENT) tf = _Period;
    ...
    json += "\"losing_trades\":" + IntegerToString(totalTrades - winningTrades) + ",";
...
//+------------------------------------------------------------------+
void GetStrategyTesterStats(double &totalProfit, int &totalTrades, int &winningTrades, double &winRate, double &profitFactor, double &maxDrawdown, double &grossProfit, double &grossLoss, double &expectedPayoff) {
    totalProfit = TesterStatistics(STAT_PROFIT);
    totalTrades = (int)TesterStatistics(STAT_TRADES);
    winningTrades = (int)TesterStatistics(STAT_PROFIT_TRADES);
